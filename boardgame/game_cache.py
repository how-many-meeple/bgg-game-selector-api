...
class DynamoDBGameCache(GameCache):
    ...
    def __init__(self, table_name: str, cache_length_seconds: int, region: str = 'us-east-1'):
        ...
        try:
            ...
            raise ImportError("boto3 is required for DynamoDB cache. Install with: pip install boto3")

    def save(self, game: BoardGame) -> None:
        """Save a game to DynamoDB cache with automatic TTL expiration"""
        try:
            ttl = int((datetime.now(timezone.utc) + timedelta(seconds=self.cache_length)).timestamp())
            item = {
                'id': str(game.id),
            ...
            log.debug(f"Cached game {game.id} ({game.name}) until {datetime.fromtimestamp(ttl)}")
